...
$(".modal_contact_btn").on('click', function () {
    ...
    if (proceed === "true") {
        ...
        if ($("#quote_result").length) {
            // alert("yes");
            output = '<div class="alert-danger" style="padding:10px 15px; margin-bottom:30px;">Por favor, rellene correctamente todos los campos.</div>';
            $("#quote_result").hide().html(output).slideDown();
            $(".modal_contact_btn i").addClass('d-none');
        }else{
            Swal.fire({
                ...
                type: 'error',
                title: 'Oops...',
                html: '<div class="text-danger">Por favor, rellena todos los campos correctamente.</div>'
            })
            $(".modal_contact_btn i").addClass('d-none');
        }
    }
